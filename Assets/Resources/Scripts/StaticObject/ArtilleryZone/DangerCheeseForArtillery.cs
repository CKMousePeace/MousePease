...
public class DangerCheeseForArtillery : MonoBehaviour
{
    ...
    //원의 크기를 seconds 시간만큼 점점 줄이는 코루틴
    {
        ...
        if(elapsedTime > seconds)
        {
            ...
            //오브젝트 풀에서 Cheese 받아옴
            CObjectPool.g_instance.ObjectPop("Cheese", new Vector3(gameObject.transform.position.x, m_Rockhigh, gameObject.transform.position.z),
            Quaternion.Euler(-90,0,0), new Vector3(5, 5, 5));

            Destroy(gameObject); //치즈 소환 후 인디케이터 제거
        }
        else
        {
