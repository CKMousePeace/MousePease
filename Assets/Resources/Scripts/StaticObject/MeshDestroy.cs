...
{
    /*  Triangle 의 각각 사이드 값을 얻어서 해당 점으로 부터 랜덤으로
        떨어진 구역의 두 점을 이어서 컷 (Pete)
        그러면 나머지 부분이 Triangle 이 아니니까 잘린(새로 만들어진)
        버텍스로부터 나머지 다른 점으로 이어줘서 Triangle 2개로 만듬  */
    ...
    public int CutCascades = 1;         //얼마나 조각낼 것인지
    public float ExplodeForce = 0;      //얼마나 큰 힘으로 터질껀지 
    public int MaxDestroyLevel = 3;     //해당 크기 이하는 자르지 않음
